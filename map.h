...
template <typename Tclef, typename Tvaleur>
void map<Tclef,Tvaleur>::afficher(typename map<Tclef,Tvaleur>::noeud* p,int niveau,std::vector<std::string>& barres,double& total,int& max)const{
    using namespace std;
    if(p==0)return;
    total+=niveau;
